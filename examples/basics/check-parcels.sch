...
<schema xmlns="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt3">
  ...
  <!-- 1 - Define the number of days a parcel's date must be before the delivery
    date and turn this into a duration: -->
  ...
  <pattern>
    <rule context="/*">
      ...
      <assert test="$total-parcels-weight le $max-weight">
        The total weight (<value-of select="$total-parcels-weight"/>) is more than the
        allowed maximum (<value-of select="$max-weight"/>)
      </assert>
    </rule>
  </pattern> 
